...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const persist = useCallback(async (next: Session | null) => { setSession(next); if (next) await SecureStore.setItemAsync(key, JSON.stringify(next)); else await SecureStore.deleteItemAsync(key); }, []);
